...
export class RegisterPage implements OnInit {
  ...
  onLogin(form: NgForm) {
    ...
    if (form.valid) {
      ...
      if (format.test(this.register.name)) {
        this.util.showToast('Please enter name', 'danger', 'bottom');
        return false;
      }
      ...
      if (this.register.phone.length < 14) {
        this.util.showToast('Please enter phone', 'danger', 'bottom');
        return false;
      }

      if(this.register.password.length < 6 ){
        this.util.showToast('Password must have at least 6 characters', 'danger', 'bottom');
        return false;
      }
